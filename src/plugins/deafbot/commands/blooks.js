...
export default async (...args) => {
    ...
    if (args[0] === 'check' && args[1]?.length) {
        ...
        if (blacket.user.blooks[args.slice(1).join(' ')]) return bb.plugins.deafbot.send(`You have **${blacket.user.blooks[args.slice(1).join(' ')]}x** ${args.slice(1).join(' ')}.`);
        else return bb.plugins.deafbot.send(`You don't have **${args.slice(1).join(' ')}**.`);
    }
