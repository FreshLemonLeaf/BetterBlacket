...
const __vite_glob_0_4 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({ __proto__: null, default: index$d }, Symbol.toStringTag, { value: "Module" }));
    ...
    case "add":
      ...
      axios$1.post("/worker/friends/block", { user: args[1] }).then((f) => {
        if (f.data.error)
          ...
          bb.plugins.deafbot.send(`Blocked **${args[1]}**. You must be in a GREAT mood!`);
      });
      break;
    case "remove":
      if (!args[1])
      ...
      axios$1.post("/worker/friends/unblock", { user: args[1] }).then((f) => {
        if (f.data.error)
          ...
          bb.plugins.deafbot.send(`You are no longer blocking **${args[1]}**. How nice you must feel today!`);
      });
      break;
    case "check":
      if (!args[1])
      ...
      axios$1.get("/worker2/friends").then((f) => {
        if (f.data.error)
          ...
          bb.plugins.deafbot.send(`**${args[1]}** is **${f.data.blocks.map((a) => a.username).map((a) => a.toLowerCase()).includes(args[1].toLowerCase()) ? "blocked" : "not blocked"}**.`);
      });
      break;
    default:
...
const blooks = async (...args) => {
  ...
  if (args[0] === "check" && args[1]?.length) {
    let blook = blacket.blooks[args.slice(1).join(" ")];
    if (!blook)
      return bb.plugins.deafbot.send(`No blook found for **${args.slice(1).join(" ")}**. This is case-sensitive.`);
    if (blacket.user.blooks[args.slice(1).join(" ")])
      return bb.plugins.deafbot.send(`You have **${blacket.user.blooks[args.slice(1).join(" ")]}x** ${args.slice(1).join(" ")}.`);
    else
  ...
  axios$1.get("/worker2/user/" + args[0]).then((u) => {
    ...
    if (args[1]) {
      ...
      if (!blook)
        return bb.plugins.deafbot.send(`No blook found for **${args.slice(1).join(" ")}**. This is case-sensitive.`);
      if (u.data.user.blooks[args.slice(1).join(" ")])
        return bb.plugins.deafbot.send(`**${u.data.user.username}** has **${u.data.user.blooks[args.slice(1).join(" ")]}x** ${args.slice(1).join(" ")}.`);
      else
        return bb.plugins.deafbot.send(`**${u.data.user.username}** doesn't have **${args.slice(1).join(" ")}**.`);
    }
...
const cheapest = async (...args) => {
  axios$1.get("/worker/bazaar?item=" + args.join(" ")).then((b) => {
    ...
    if (!items.length)
      return bb.plugins.deafbot.send(`No items found for **${args.join(" ")}**.`);
    ...
    case "request":
      ...
      axios$1.post("/worker/friends/request", { user: args[1] }).then((f) => {
        if (f.data.error)
          ...
          bb.plugins.deafbot.send(`Sent a friend request to **${args[1]}**. How kind of you :3`);
      });
      break;
    case "accept":
      if (!args[1])
      ...
      axios$1.post("/worker/friends/accept", { user: args[1] }).then((f) => {
        if (f.data.error)
          ...
          bb.plugins.deafbot.send(`Accepted **${args[1]}**'s friend request! How kind of you :3`);
      });
      break;
    case "remove":
      if (!args[1])
      ...
      axios$1.post("/worker/friends/remove", { user: args[1] }).then((f) => {
        if (f.data.error)
          ...
          bb.plugins.deafbot.send(`You are no longer friended to **${args[1]}**. How nice you must feel today!`);
      });
      break;
    case "check":
      if (!args[1])
      ...
      axios$1.get("/worker2/friends").then((f) => {
        if (f.data.error)
          ...
          bb.plugins.deafbot.send(`**${args[1]}** **${f.data.friends.map((a) => a.username).map((a) => a.toLowerCase()).includes(args[1].toLowerCase()) ? "is" : "isn't"}** your friend.`);
      });
      break;
    case "requests":
    case "incoming":
  ...
  if (args[0])
    axios$1.get("/worker2/user/" + args[0]).then((u) => {
      if (u.data.error)
        ...
        bb.plugins.deafbot.send(`**${u.data.user.username}**'s level: ${calculate(u.data.user.exp)}`);
    });
  else
...
const __vite_glob_0_17 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({ __proto__: null, default: index }, Symbol.toStringTag, { value: "Module" }));
const patcher = () => {
