...
const createPlugin = ({
  ...
  if (!title || !authors?.length || !onLoad && !onStart && !patches)
...
const index$f = () => createPlugin({
  ...
  onStart: () => {
    if (!location.pathname.startsWith("/market"))
      ...
      console.log("Advanced Opener started!");
    bb.plugins.massopen = {};
...
const cheapest = async (...args) => {
  axios.get("/worker/bazaar?item=" + args.join(" ")).then((b) => {
    if (b.error)
      return bb.plugins.deafbot.send(`Error fetching bazaar: **${b.reason}**`);
...
const loadThemes = async (single) => {
  ...
  for (let theme of themes) {
    axios.get(theme).then(async (res) => {
      ...
      if (meta.autoImportant === "true" || !meta.autoImportant)
        data = data.replaceAll(/([^;{}]*:[^;{}]*);/g, "$1 !important;");
